      ...
      <main className="max-w-6xl mx-auto px-4 py-16 flex flex-col items-center">
        {step <= 6 ? (
          ...
          <div className="animate-in fade-in zoom-in-95 duration-1000 max-w-5xl mx-auto text-right px-3 sm:px-4" dir="rtl">
              ...
              {!isPurchased && (
                <div className="mb-6 p-5 sm:p-6 rounded-2xl border-2 border-dashed border-[#d4af37] bg-gradient-to-br from-[#001a33] to-[#1e3a5f] text-center shadow-xl">
                  <Lock size={28} className="text-[#d4af37] mx-auto mb-3" />
                  <h4 className="text-base sm:text-xl font-black text-white mb-1">דוח תמהילים אופטימלי נעול</h4>
                  <p className="text-slate-300 font-bold text-xs sm:text-sm mb-4 leading-relaxed">הפקת התמהילים המדויקים ופירוט הריביות דורשת פתיחת תיק במיקוד משכנתאות.</p>
                  <button onClick={handlePurchaseClick} className="bg-[#d4af37] text-[#001a33] px-6 sm:px-10 py-3 rounded-xl font-black text-sm sm:text-lg shadow-lg hover:bg-white transition-all transform hover:scale-105 active:scale-95 flex items-center gap-2 mx-auto">
                    <span>רכוש דוח מלא — ₪499 + מע"מ</span>
                    <ChevronLeft size={18} />
                  </button>
                </div>
              )}
              ...
              <div className="relative mb-6 sm:mb-10">
                {!isPurchased && (
                  <div className="absolute inset-0 z-20 backdrop-blur-xl bg-white/40 rounded-xl sm:rounded-[2rem]" />
                )}
