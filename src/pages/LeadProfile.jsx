...
import {
  User, Coins,
  BadgeCheck, Download,
  Clock, CheckCircle, ArrowLeft, FileJson, Send, Sparkles
} from 'lucide-react';
...
function InfoRow({ label, value }) {
  ...
  return (
    <div className="flex justify-between items-center py-2.5 border-b border-mist-100 last:border-0">
      <span className="text-mist-500 text-sm font-medium">{label}</span>
      <span className="font-bold text-[#0C084A] text-sm">{value}</span>
    </div>
  );
}
...
function Section({ title, icon: SectionIcon, children }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-mist-100 p-6">
      <h3 className="font-black text-[#0C084A] text-base mb-4 flex items-center gap-2">
        <SectionIcon size={18} className="text-[#0153F4]" />
        {title}
      </h3>
      {children}
    </div>
...
function LeadProfileContent() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-mist-50 font-sans">
      <p className="text-mist-500 font-bold">טוען תיק לקוח...</p>
    </div>
  );

  if (!lead) return (
    <div className="min-h-screen flex items-center justify-center bg-mist-50 font-sans" dir="rtl">
      <div className="text-center">
        <p className="text-mist-500 font-bold mb-4">הליד לא נמצא</p>
        <a href={createPageUrl('AdminDashboard')} className="text-[#0C084A] underline">חזור לפאנל</a>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-mist-50 font-sans" dir="rtl">
      {/* Header */}
      <div className="bg-gradient-to-l from-[#0C084A] to-[#06042A] px-6 py-5 shadow-md">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[#0153F4] text-xs font-semibold">תיק לקוח מלא</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={exportJson}
              className="flex items-center gap-2 bg-[#0153F4] text-[#0C084A] px-4 py-2 rounded-xl font-black text-sm hover:bg-[#4C82FF] transition-all"
            >
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-l from-[#0C084A]/10 to-[#0153F4]/10 border-2 border-[#0153F4]/40 rounded-2xl p-5 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold text-mist-500 mb-1">מזהה תיק — להעברה למערכת 2</p>
            <p className="font-black text-[#0C084A] text-lg font-mono">{lead.id}</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={copyId}
              className="flex items-center gap-2 bg-white border-2 border-[#0C084A] text-[#0C084A] px-4 py-2 rounded-xl font-bold text-sm hover:bg-[#0C084A] hover:text-white transition-all"
            >
            ...
            <button
              onClick={exportJson}
              className="flex items-center gap-2 bg-[#0C084A] text-white px-4 py-2 rounded-xl font-bold text-sm hover:bg-[#0153F4] transition-all"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: 'סכום משכנתא', value: formatCurrency(lead.loanAmount), suffix: '', color: 'text-[#0C084A]' },
            { label: 'הכנסה מוכרת', value: formatCurrency(lead.netIncome), suffix: '', color: 'text-[#0C084A]' },
          ].map((m, i) => (
            <div key={i} className="bg-white rounded-2xl p-4 shadow-sm border border-mist-100 text-center">
              <p className={`text-2xl font-black ${m.color}`}>{m.value}<span className="text-sm">{m.suffix}</span></p>
              <p className="text-xs text-mist-500 font-semibold mt-1">{m.label}</p>
            </div>
          ))}
        </div>
        ...
        {lead.aiAnalysis && (
          <Section title="ניתוח AI מלא" icon={Sparkles}>
            <p className="text-mist-700 text-sm leading-relaxed whitespace-pre-line">{lead.aiAnalysis}</p>
          </Section>
        )}

        {/* Status Update */}
        <div className="bg-white rounded-2xl shadow-sm border border-mist-100 p-6 mt-6">
          <h3 className="font-black text-[#0C084A] mb-4 flex items-center gap-2">
            <Clock size={18} className="text-[#0153F4]" /> עדכון סטטוס תיק
          </h3>
            ...
            {Object.entries(STATUS_LABELS).map(([val, { label, color }]) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-bold border-2 transition-all ${
                  status === val
                    ? 'border-[#0C084A] bg-[#0C084A] text-white'
                    : `border-transparent ${color} hover:border-[#0C084A]/30`
                }`}
              >
                {label}
              </button>
        ...
        {/* Export CTA */}
        <div className="mt-6 bg-gradient-to-l from-[#0C084A] to-[#06042A] rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-right">
          <div>
            <h3 className="text-white font-black text-lg">מוכן להעברה למערכת 2?</h3>
            <p className="text-[#0153F4] text-sm mt-1">ייצא את תיק הלקוח המלא והעבר למערכת העלאת המסמכים וההגשה</p>
          </div>
          <button
            onClick={exportJson}
            className="flex items-center gap-3 bg-[#0153F4] text-[#0C084A] px-8 py-4 rounded-2xl font-black text-base hover:bg-[#4C82FF] transition-all shadow-lg flex-shrink-0"
          >
