...
import { Star, Sparkles, ChevronDown, ChevronUp, TrendingDown, ShieldCheck, Zap } from 'lucide-react';
...
export default function MixTable({ title, subtitle, tracks, totalPmt, isRecommended, mixType = 'recommended', loanAmount, durationYears, saving }) {
  ...
  return (
    ...
    >
      {/* פס צבע עליון */}
      <div className={`h-1.5 w-full ${isRecommended ? 'bg-gradient-to-r from-[#0153F4] to-[#4C82FF]' : 'bg-mist-200'}`} />

      {isRecommended && (
        <div className="absolute top-3 left-3 z-10">
          <div className="bg-gradient-to-r from-[#0153F4] to-[#4C82FF] text-white px-3 py-1 rounded-full font-bold text-[10px] shadow flex items-center gap-1">
            <Sparkles size={10} />
            מומלץ
          </div>
        </div>
      ...
      {/* כותרת + סכום */}
      <div className={`p-4 sm:p-5 ${isRecommended ? 'bg-gradient-to-br from-[#0C084A] to-[#06042A]' : 'bg-mist-50'}`}>
        <div className="flex items-center gap-2 mb-2">
          <Icon size={16} className={isRecommended ? 'text-[#0153F4]' : 'text-mist-500'} />
          <h4 className={`text-sm sm:text-base font-black leading-tight ${isRecommended ? 'text-white' : 'text-[#0C084A]'}`}>{title}</h4>
        </div>
        <p className={`text-[11px] mb-3 leading-snug ${isRecommended ? 'text-white/70' : 'text-mist-600'}`}>{subtitle || meta.strategy}</p>
        <div className={`rounded-xl px-4 py-3 text-center ${isRecommended ? 'bg-white/15 border border-white/20' : 'bg-[#0C084A]'}`}>
        ...
        {saving != null ? (
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-center">
            <div className={`rounded-lg px-2 py-1.5 ${isRecommended ? 'bg-white/10' : 'bg-white border border-mist-200'}`}>
              <p className={`text-[9px] font-semibold ${isRecommended ? 'text-white/60' : 'text-mist-400'}`}>סה"כ ריבית</p>
              <p className={`text-xs font-black ${isRecommended ? 'text-red-300' : 'text-red-500'}`}>₪{formatCurrency(Math.floor(totalInterest))}</p>
            </div>
            <div className={`rounded-lg px-2 py-1.5 ${isRecommended ? 'bg-white/10' : 'bg-white border border-mist-200'}`}>
              <p className={`text-[9px] font-semibold ${isRecommended ? 'text-white/60' : 'text-mist-400'}`}>סה"כ לתשלום</p>
              <p className={`text-xs font-black ${isRecommended ? 'text-white' : 'text-[#0C084A]'}`}>₪{formatCurrency(Math.floor(totalPayment))}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 p-3 sm:p-4 space-y-2">
        {tracks.map((track, idx) => (
            <div key={idx} className="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-mist-50 border border-mist-100 hover:border-[#0153F4]/30 transition-colors">
              <div className="flex-1 min-w-0">
                <p className="font-bold text-xs text-[#0C084A] leading-tight truncate">{track.name}</p>
                <p className="text-[10px] text-mist-400 leading-tight truncate">{track.desc}</p>
              </div>
              <div className="text-center flex-shrink-0 px-2">
                <p className="text-[#0153F4] font-black text-xs">{(track.rate * 100).toFixed(2)}%</p>
                <p className="text-mist-400 text-[9px]">{track.years} שנה</p>
              </div>
              <div className="text-left flex-shrink-0">
                <p className="font-black text-sm text-[#0C084A]">₪{formatCurrency(Math.floor(track.pmt))}</p>
                <p className="text-[9px] text-mist-400 text-center">לחודש</p>
              </div>
            </div>
          ))}
          {/* סה"כ חודשי בולט */}
      ...
      <div className="border-t border-mist-100">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-3 hover:bg-mist-50 transition-colors text-right"
        >
          <span className="text-[11px] font-bold text-[#0C084A]">למה בחרנו תמהיל זה?</span>
          {expanded ? <ChevronUp size={14} className="text-mist-400" /> : <ChevronDown size={14} className="text-mist-400" />}
        </button>
        {expanded && (
          <div className="px-4 pb-4 space-y-3">
            <p className="text-xs text-mist-700 leading-relaxed">{meta.rationale}</p>
