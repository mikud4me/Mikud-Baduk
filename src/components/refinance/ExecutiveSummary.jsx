...
export default function ExecutiveSummary({ analysisResult, headline, externalTrigger, onTriggerDone }) {
  ...
  const { currentLoan, savings, surgicalAnalysis, mixes, conclusionText, snapshotHeader, dualStrategy } = analysisResult;
  ...
  const trackColors = ['#0153F4', '#2563eb', '#16a34a', '#7c3aed'];
  ...
  // ── styles ── (unchanged from source — this is the printable report's own light
  // palette, already white/slate-based; only the gold "#C5A059/#D4AF37" recommended-mix
  // accent below was swapped to brand blue, everything else is left as tuned for
  // html2pdf pagination/fidelity)
  const S = {
    ...
    sectionTitle: () => ({ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }),
    ...
    mixCard: (isRec) => ({
      border: isRec ? '2.5px solid #0153F4' : '1.5px solid #e2e8f0',
      borderRadius: '10px',
      overflow: 'hidden',
      background: isRec ? '#eaf1ff' : '#f8fafc',
      pageBreakInside: 'avoid',
    }),
    mixHeader: (isRec) => ({
      background: isRec ? 'linear-gradient(90deg, #0141C2, #0153F4)' : '#1e293b',
      padding: '10px 14px',
      color: 'white',
      fontWeight: '800',
      fontSize: '13px',
          ...
          {mixes?.length > 0 && (
            <>
              ...
              <div style={S.page}>
                ...
                {/* Page header */}
                <div style={{ ...S.header, borderBottomColor: '#0153F4' }}>
                  <div>
                    <div style={{ fontSize: '9px', color: '#94a3b8', letterSpacing: '2px', marginBottom: '2px' }}>MIKUD MORTGAGES · מיקוד משכנתאות</div>
                ...
                <div style={{ fontSize: '11px', color: '#475569', marginBottom: '20px', lineHeight: '1.6', background: '#f8fafc', borderRadius: '8px', padding: '10px 14px', border: '1px solid #e2e8f0' }}>
                  להלן 3 תמהילים מומלצים עבורכם, המחושבים על בסיס ריביות השוק העדכניות. כל תמהיל מייצג גישה שונה לאיזון בין יציבות לחיסכון. <strong>התמהיל המומלץ</strong> מסומן ומודגש.
                </div>
                    ...
                    return (
                      <div key={idx} style={S.mixCard(isRec)}>
                        ...
                        <div style={{ padding: '12px' }}>
                          ...
                          <div style={{ background: isRec ? '#eaf1ff' : '#f1f5f9', borderRadius: '8px', padding: '10px', textAlign: 'center', marginBottom: '10px', border: isRec ? '1px solid #abc7ff' : '1px solid #e2e8f0' }}>
                            <div style={{ fontSize: '8px', color: '#64748b' }}>החזר חודשי</div>
                            <div style={{ fontSize: '20px', fontWeight: '900', color: isRec ? '#0141C2' : '#0f172a' }}>₪{mix.total_monthly_payment?.toLocaleString()}</div>
                ...
                <table style={{ ...S.table, marginBottom: '20px' }}>
                  ...
                  <tbody>
                    {mixes.filter(Boolean).slice(0, 3).map((mix, i) => {
                      ...
                      const rl = riskLabels[mix.risk_level] || { label: 'מותאם', color: '#0f172a' };
                      return (
                        <tr key={i} style={{ background: isRec ? '#eaf1ff' : i % 2 === 0 ? 'white' : '#f8fafc' }}>
